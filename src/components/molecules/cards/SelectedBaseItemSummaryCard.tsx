...
export const SelectedBaseItemSummaryCard = ({
  selectedBaseItem,
}: SelectedBaseItemSummaryCardPropsType): ReactElement => {
  ...
  return (
    <div className="flex flex-col gap-1 rounded-xl border border-zinc-200 bg-zinc-50/80 px-4 py-3 dark:border-zinc-800 dark:bg-zinc-900/50">
      <div className="text-lg font-semibold text-amber-700 dark:text-amber-400">
        <CatalogBaseName baseItemKey={selectedBaseItem.baseItemKey} />
      </div>
      <div className="text-xs text-zinc-500 dark:text-zinc-500">
        <CatalogItemClassLabel itemClassKey={selectedBaseItem.itemClassKey} />
      </div>
      {resultRecord &&
