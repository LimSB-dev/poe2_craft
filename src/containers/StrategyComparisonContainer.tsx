...
import { Button, ReservedStatusRegion } from "@/components/atoms";
import {
  SiteTopBar,
...
import {
  ...
  STRATEGY_COMPARISON_TRIALS_QUERY,
...
const fetchStrategyJson = async (params: {
  ...
  const search = new URLSearchParams({
    trials: String(STRATEGY_COMPARISON_TRIALS_QUERY),
...
export const StrategyComparisonContainer = (): ReactElement => {
  ...
  const [minGoodMods, setMinGoodMods] = useState<number>(
    STRATEGY_COMPARISON_DEFAULT_MIN_GOOD_MODS,
  );
  const [budget, setBudget] = useState<number>(STRATEGY_COMPARISON_DEFAULT_BUDGET_MAX_CHAOS);
  ...
  return (
    <>
      <header>
        <SiteTopBar pageHeading={t("title")} />
      </header>
          ...
          <div className="flex flex-col sm:flex-row sm:flex-wrap gap-4 sm:items-end">
            <div className="flex flex-col gap-1.5 flex-1 min-w-[10rem]">
              ...
              <input
                ...
                type="number"
                min={STRATEGY_COMPARISON_MIN_GOOD_MODS_MIN}
                max={STRATEGY_COMPARISON_MIN_GOOD_MODS_MAX}
                value={minGoodMods}
                onChange={(e) => {
                  ...
                  if (Number.isFinite(next)) {
                    setMinGoodMods(
                      Math.min(
                        Math.max(next, STRATEGY_COMPARISON_MIN_GOOD_MODS_MIN),
                        STRATEGY_COMPARISON_MIN_GOOD_MODS_MAX,
                      ),
                    );
                  }
            ...
            <div className="flex flex-col gap-1.5 flex-1 min-w-[10rem]">
              ...
              <input
                id="budget-chaos"
                type="number"
                min={STRATEGY_COMPARISON_BUDGET_MIN}
                max={STRATEGY_COMPARISON_BUDGET_MAX}
                value={budget}
                onChange={(e) => {
                  const next = Number.parseInt(e.target.value, 10);
                  if (Number.isFinite(next)) {
                    setBudget(
                      Math.min(
                        Math.max(next, STRATEGY_COMPARISON_BUDGET_MIN),
                        STRATEGY_COMPARISON_BUDGET_MAX,
                      ),
                    );
                  }
            ...
            <Button
              ...
              disabled={loading}
              variant="outline"
              className="border-amber-600/80 dark:border-amber-500/60 bg-amber-500/15 dark:bg-amber-500/10 text-amber-950 dark:text-amber-100 sm:shrink-0"
            >
              {t("form.run")}
            </Button>
          </div>
